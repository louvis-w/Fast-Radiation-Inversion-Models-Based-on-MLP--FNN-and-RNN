...
class Config:
    ...
    task_output_dim = n_levels  # 每个任务输出80维（80层）
    
    # 轻量化模型配置（适配单任务）
    hidden_dims = [1024, 512, 256]  # 降低模型复杂度
    dropout_rate = 0.1
    batch_norm_eps = 1e-5
...
class ERA5SingleTaskDataset(Dataset):
    def __init__(self, ml_data, rad_data, task_var, normalize=True):
        ...
        # 第一步：裁剪为底部80层数据
        # ml_data.shape = (N, 137*16) → 取前80层：0~80*16-1
...
def load_80level_data(ml_path, rad_path):
    ...
    print(f"80层参数维度: {ml_data.shape}, 亮温维度: {rad_data.shape}")
...
def train_single_task(task_var, ml_data, rad_data):
    ...
    print(f"\n任务 {task_var} 训练完成！最佳模型保存至: best_mlp_80level_{task_var}.pth")
    return model_path
...
if __name__ == "__main__":
    ...
    print(f"\n温度t预测示例（80层）:")
    print(f"真实值（第0层）: {true_t_denorm[0]:.4f}")
    print(f"预测值（第0层）: {pred_t_denorm[0,0]:.4f}")